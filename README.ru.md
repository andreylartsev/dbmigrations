...
| `r`      | Пере-опрос состояния схемы/репозитория (Refresh); после опроса в лог выводится сводка (доступно команд, репозиторий git) |
| `q`      | Выход |
...
Строки лога, содержащие ссылку на git `(OID: …)` (например, в recent-changes и диффах `verify`), работают как ссылки: щелчок по такой строке открывает модальное окно с содержимым файла, прочитанным прямо из git-репозитория (`git cat-file`); закрыть — `escape`/`q`/`enter`. Если git недоступен, появится уведомление с пояснением.
