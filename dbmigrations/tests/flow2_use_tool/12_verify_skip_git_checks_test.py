...
def test_dbmigration_verify_skip_git_checks_success(esbdb_cfg):
    ...
    output_sql_file = esbdb_cfg.TESTS_DIR.joinpath(unique_filename)
    ...
    try:
        ...
        assert f"set_config('search_path', '{esbdb_cfg.TARGET_SCHEMA}', false);" in sql_content
        assert "create table t1" in sql_content
        assert "insert into t1 values (1);" in sql_content
        assert f"INSERT INTO \"{esbdb_cfg.TARGET_SCHEMA}\".dbmigration_versions" in sql_content
